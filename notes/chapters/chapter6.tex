...
\par In an Euclidean space, we could define the \emph{luminosity distance} as
\begin{equation*}
...
\begin{enumerate}
    ...
    \begin{equation}
        \chi = \int_{0}^{\tilde{r}_{s}}\frac{\diff{r}}{\sqrt{1-kr^{2}}} = f_{k}^{-1}(\tilde{r}_{s}) = \left\{
            \begin{array}{ll}
                \tilde{r}_{s} & k=0\\
                \arcsin(\tilde{r}_{s}) & k=1\\
                \text{arcsinh}(\tilde{r}_{s}) & k=-1
            \end{array}
        \right.
    \end{equation}
    \item \emph{Correction to the concept of luminosity}: In an expanding Universe, we have to be a little more cautious with what we refer to as "intrinsic luminosity." The necessary steps to be taken are explained in detail in the following.
\end{enumerate}
...
\end{equation*}
so that by direct comparison with our initial equation, we find that the \emph{luminosity distance} is
\begin{equation}
...
\end{equation*}
where we've defined $E(z) = H(z)/H_{0}$. Still, it is more common to speak in terms of $\Omega_{k,0} = -k/a_{0}^{2}H_{0}^{2}$, which can be measured. We can therefore write the luminosity in terms of measurable cosmological parameters as 
\begin{equation}
    \boxed{
    d_{L}(z) = (1+z)\frac{H_{0}^{-1}}{\sqrt{|\Omega_{k,0}|}}f_{k}\left[\sqrt{|\Omega_{k,0}|}\int \frac{\text{d}z'}{E(z')}\right]
    }
    \label{eq:general_lumdist}
\end{equation} \marginpar{Recall that the spatial density parameter $\Omega_{k,0}$ must abide the consistency relation $$ 1 = \sum_{i}\Omega_{i,0} + \Omega_{k,0} $$}
...
\end{equation}
If $r_{s} < r_{\star}$, the event will never be observed, meaning there could be regions of spacetime that are impossible to "communicate" with. To see if the integral can actually be finite, you will usually check 
\begin{equation}
    r_{\star} = -\frac{1}{AH}e^{-Ht}\Big|^{\infty}_{t_{\star}} < \infty
\end{equation} \marginpar{We're assuming a flat Universe $k=0$ for which $$ a(t) = A e^{Ht}$$}
For example, if the EoS has $w>-1/3$, $r_{\star}\to \infty$ and the horizon is not properly defined.

\section{Flatness Problem}

Consider a Universe with vanishing Cosmological Constant $\Lambda = 0$ which is also such that Einstein's equation is \emph{always} valid, also at $t=0$. We can define
\begin{equation}
    \Omega_{k} = 1-\Omega = \frac{\rho_{c}-\rho}{\rho_{c}} = -\frac{k}{a^{2}H^{2}}
\end{equation}
Now assume that $k\neq 0$ and $\Omega_{k}$ can evolve independently but along the dominant contribution to the cosmological fluid. If
\begin{itemize}
...
\end{equation*}
and the distance from unity would only decrease if we went even further back in time. Setting then those orders of magnitude as initial conditions implies a \emph{huge fine-tuning problem}.
...
\end{equation}
If we want the final result to be independent from the particular cosmology that we've chosen \marginpar{Here the prime derivative is with respect to $x$, so $$ \diff{t} = \frac{1}{H}\diff{x} $$ and not conformal time.}
\begin{align}
...
\par The flatness problem is thus resolved if we postulate a Universe that accelerates for enough time. Such period of time is called \emph{inflationary epoch} or just \emph{inflation}. If during the inflation $H \sim \text{const,}$ then 
\begin{equation}
...
Another problem remains to be addressed. We know that photons decouple from ordinary matter at $z\approx 1100$ (decoupling/last scattering). Currently, within our particle horizon, we observe a roughly homogeneous temperature distribution, which is that of the CMB.
...
but 
\begin{equation}
    \Big|\frac{\Omega_{k}(t_{f})}{\Omega_{k}(t_{i})}\Big| = e^{-2N} \iff |\Omega_{k}(t_{0})| = e^{-2N}|\Omega_{k}(t_{i})|\left(\frac{a_{f}^{2}H_{i}^{2}}{a_{0}^{2}H_{0}^{2}}\right)
\end{equation}
...
\begin{figure}[h!]
    ...
    \label{fgr:inflation}
\end{figure}
\par The existence of an epoch of accelerated expansion corresponds to having $p < -\rho/3$ and thus violates the \emph{strong energy condition}. \marginpar{Given a timelike vector $t^{\mu}$, the strong energy condition requires that $$ T_{\mu\nu}t^{\mu}t^{\nu} \geq \frac{1}{2}T^{\lambda}_{\;\lambda}t^{\sigma}t_{\sigma} $$ for all timelike vectors. Equivalently, it requires $p+\rho \geq 0$ and $\rho + 3p \geq 0$.}
...
\begin{figure}[h!]
    ...
    \caption{When the scalar field $\varphi$ is near region (a), the energy density will remain nearly constant, $\rho\approx \rho_{f}$, even as the Universe expands. Moreover, cosmic expansion acts like a frictional drag, slowing the motion of $\varphi$: Even near regions (b) and (d), $\varphi$ behaves more like a marble moving in a bowl of molasses, slowly creeping down the side of its potential, rather than like a marble sliding down the inside of a polished bowl. During this period of "slow roll," $\rho$ remains nearly constant. Only after $\varphi$ has slid most of the way down its potential will it begin to oscillate around its minimum, as in region (c), ending inflation. Credits: \href{https://ned.ipac.caltech.edu/level5/March05/Guth/Guth1.html}{this website}.}
\end{figure}
\par In short, they were postulating the existesnce of a scalar field $\varphi$. The associated potential $U(\varphi)$ had to present a local maximum where the phase transition occurred (so at the beginning of inflation), and a local minimum where $U(\varphi)\approx \text{const.} = \Lambda$, hence a metastable de Sitter phase during which tunnel effect occurred towards true vacuum. This was the so-called \emph{old inflation}.
...
\begin{figure}[h!]
    \centering 
    \includegraphics[width=0.75\textwidth]{img/slowroll.png}
...
\begin{align}
    ...
    p_{\varphi} &= \frac{T^{i}_{\;i}}{3} = \frac{\dot{\varphi}^{2}}{2}-V(\varphi)-\frac{(\nabla\varphi)^{2}}{6a^{2}}
\end{align}
...
\begin{align}
    ...
    \frac{T^{i}_{\;i}}{3} &= p_{\varphi} = \frac{\dot{\varphi}_{0}^{2}}{2} - V(\varphi_{0})
\end{align}
...
Consider now 
\begin{equation}
    \ddot{\varphi} = -\frac{\text{d}}{\text{d}t}\left(\frac{V'}{3H}\right) \implies \ddot{\varphi} = -\frac{V''\dot{\varphi}}{3H} + \frac{V'\dot{H}}{3H^{2}}
\end{equation}
